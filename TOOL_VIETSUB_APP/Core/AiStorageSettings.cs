...
public sealed class AiStorageService : IDisposable
{
    ...
    internal const string MigrationDirectoryName = ".tool-vietsub-migration";
        ...
        bool migrateExisting)
    {
            ...
            || IsWithin(destination, source.RootPath))
        {
            throw new LocalModelException(
                "AI_STORAGE_INVALID",
                "Hãy chọn một thư mục riêng, ví dụ D:\\TOOL_VIETSUB_AI; không chọn gốc ổ đĩa, Projects hoặc thư mục nằm trong vị trí AI hiện tại.");
        }
    ...
    private static string GetRecommendedPath()
    {
        var preferredDrive = DriveInfo.GetDrives()
            ...
            .FirstOrDefault();
        return Path.Combine(preferredDrive?.RootDirectory.FullName ?? "D:\\", "TOOL_VIETSUB_AI");
    }
