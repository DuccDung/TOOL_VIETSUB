...
public sealed class DesktopApiClient : IDisposable
{
    ...
    {
        var configuredUrl = Environment.GetEnvironmentVariable("SUBVID_API_BASE_URL");
        var baseUri = new Uri(
        ...
        {
            throw new InvalidOperationException("SubVid API must use HTTPS outside localhost.");
        }
        ...
        _httpClient.DefaultRequestHeaders.UserAgent.ParseAdd("SubVid-App/1.0");
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (!response.IsSuccessStatusCode || envelope is null || !envelope.Success || envelope.Data is null)
        {
            throw new ApiClientException(
                ...
                envelope?.Error?.Message ?? "Không thể kết nối tới máy chủ SubVid.",
