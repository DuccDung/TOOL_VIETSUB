...
public sealed class PaddleOcrIntegrationTests : IDisposable
{
    ...
    public async Task Recognize_LocalPaddleRuntime_ReadsGeneratedFrame()
    {
        ...
        Assert.Contains("VIETSUB", string.Join(' ', lines.Select(line => line.Text)), StringComparison.OrdinalIgnoreCase);
    }
    ...
    public async Task OcrJob_WithRealVideo_CreatesTimedSubtitleTrack()
    {
        ...
        Assert.Contains("VIETSUB", string.Join(' ', cues.Select(cue => cue.OriginalText)), StringComparison.OrdinalIgnoreCase);
