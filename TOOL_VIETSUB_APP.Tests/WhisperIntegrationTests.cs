...
public sealed class WhisperIntegrationTests
{
    ...
    {
        var modelRoot = Environment.GetEnvironmentVariable("TOOL_VIETSUB_TEST_WHISPER_MODEL_ROOT");
        var wavePath = Environment.GetEnvironmentVariable("TOOL_VIETSUB_TEST_SPEECH_WAV");
        if (string.IsNullOrWhiteSpace(modelRoot) || string.IsNullOrWhiteSpace(wavePath))
        {
            return;
        }

        var previous = Environment.GetEnvironmentVariable("TOOL_VIETSUB_MODEL_ROOT");
        Environment.SetEnvironmentVariable("TOOL_VIETSUB_MODEL_ROOT", modelRoot);
        try
        {
            var paths = new AppPaths(Path.Combine(Path.GetTempPath(), "TOOL_VIETSUB_WHISPER_TEST"));
        ...
        {
            Environment.SetEnvironmentVariable("TOOL_VIETSUB_MODEL_ROOT", previous);
        }
    }
}
