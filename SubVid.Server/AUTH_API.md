# SubVid authentication and account API
...
1. Deploy `SubVid_V1.sql`.
2. Deploy `SubVid_AUTH_V2.sql`.
3. Deploy `SubVid_REGISTRATION_V3.sql`.
4. Deploy `SubVid_QUOTA_V4.sql`.
5. Deploy `SubVid_PASSWORD_RESET_V5.sql`.
5. Configure the registration and SMTP secrets described below.
6. Start `SubVid.Server` with the HTTPS launch profile.
7. Open `https://localhost:7198/Setup`.
8. Create the first administrator account.
9. Start `SubVid.App` and sign in or create a USER account.
...
key through the `SUBVID_JWT_SIGNING_KEY` environment variable. Do not add
...
this with `SUBVID_API_BASE_URL`; non-local URLs must use HTTPS.

## Registration secret configuration

Development values belong in .NET User Secrets for `SubVid.Server`:

```powershell
dotnet user-secrets set "Registration:OtpSecret" "<random-secret-at-least-32-characters>" --project SubVid.Server/SubVid.Server.csproj
dotnet user-secrets set "Smtp:User" "<smtp-account>" --project SubVid.Server/SubVid.Server.csproj
dotnet user-secrets set "Smtp:Pass" "<smtp-app-password>" --project SubVid.Server/SubVid.Server.csproj
```

Production must set `SUBVID_REGISTRATION_OTP_SECRET`,
`SUBVID_SMTP_USER`, and `SUBVID_SMTP_PASSWORD`. SMTP host, port,
