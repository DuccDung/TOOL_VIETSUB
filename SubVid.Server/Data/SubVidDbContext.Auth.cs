...
public partial class SubVidDbContext
{
    ...
    partial void OnModelCreatingPartial(ModelBuilder modelBuilder)
    {
        ...
        {
            entity.HasIndex(item => item.EmailNormalized, "UX_password_reset_challenges_pending_email")
                .IsUnique()
                .HasFilter("([status_code]='PENDING')");
            entity.HasIndex(
                item => new { item.StatusCode, item.ExpiresAtUtc },
                "IX_password_reset_challenges_expiry");
            entity.Property(item => item.ChallengeId)
                .HasDefaultValueSql("(newsequentialid())", "DF_password_reset_challenges_id");
            entity.Property(item => item.EmailNormalized)
                .HasComputedColumnSql("(upper(ltrim(rtrim([email]))))", true);
            entity.Property(item => item.StatusCode)
                .HasDefaultValue("PENDING", "DF_password_reset_challenges_status");
            entity.Property(item => item.CreatedAtUtc)
                .HasDefaultValueSql("(sysutcdatetime())", "DF_password_reset_challenges_created_at");
            entity.Property(item => item.UpdatedAtUtc)
                .HasDefaultValueSql("(sysutcdatetime())", "DF_password_reset_challenges_updated_at");
            entity.Property(item => item.RowVersion).IsRowVersion().IsConcurrencyToken();
            entity.HasOne(item => item.User)
                .WithMany()
                .HasForeignKey(item => item.UserId)
                .OnDelete(DeleteBehavior.SetNull)
                .HasConstraintName("FK_password_reset_challenges_user");
        });
    }
}
